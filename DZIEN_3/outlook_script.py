...
def run_ol_Script(outdest, filefmt, olreadfolder, olprocessedfolder, gui_entry, proc):
    ...
    for msg in list(messages):
        ...
        if proc == 'olbody':
            ...
            temp_filename = os.path.normpath(
                os.path.join(outdest, f'{msg.Subject} {msg.CreationTime.strftime("%Y%m%d")} .csv'))
            b_processed = True
...
if __name__ == "__main__":
    ...
    if (not b_olatt and not b_olbody):
        sys.exit(
            'No process choice made, choose between ol attachments saver (--olatt) and ol mail body saver (--olbody)!')
    if args.outdest == '':
